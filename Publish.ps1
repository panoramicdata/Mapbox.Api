...
# Progress is written to the information stream rather than the host, so it can be captured,
# redirected or suppressed by a caller. -InformationAction Continue keeps it visible by default.
...
if ($runExitCode -ne 0) {
	Write-Status ""
	Write-Status "The release run did not succeed: https://github.com/$repoFullName/actions/runs/$runId"
	...
	if ($LASTEXITCODE -eq 0 -and $jobId) {
		...
		if ($LASTEXITCODE -eq 0 -and $annotation) {
			Write-Status "Reason: $annotation"
		}
	}

	Write-Status ""
	Write-Status "Tag $version is pushed but no package was published. Once the cause is fixed:"
	Write-Status "  gh run rerun $runId --repo $repoFullName --failed"
	exit 1
}

Write-Status "Package $version published."